...
export function createDenoBackend() {
  ...
  return {
    dlopen(path, symbols) {
      const denoPath = path instanceof URL ? fileURLToPath(path) : path;

      const denoSymbols = Object.fromEntries(
        Object.entries(symbols).map(([name, def]) => [
          name,
          {
            parameters: (def.args ?? []).map(toDenoFFIType),
            result: toDenoFFIType(def.returns ?? FFIType.void),
            nonblocking: def.threadsafe ?? false,
          },
        ]),
      );

      const library = DenoGlobal.dlopen(denoPath, denoSymbols);
      const callbacks = new Set();
      let closed = false;

      const wrappedSymbols = Object.fromEntries(
        Object.entries(library.symbols).map(([name, symbolFn]) => {
      ...
      return {
        ...
        createCallback(callback, definition) {
          ...
          const callbackSignal = {
            parameters: (definition.args ?? []).map(toDenoFFIType),
    ...
    ptr(value) {
      ...
      const basePointer = DenoGlobal.UnsafePointer.of(buffer);
    ...
    toArrayBuffer(pointer, offset, length) {
      const ptrBigInt = unwrapToBigInt(pointer) + toNodePointerOffset(offset);
      const denoPtr = DenoGlobal.UnsafePointer.create(ptrBigInt);
      if (denoPtr === null) return new ArrayBuffer(0);
      const view = new DenoGlobal.UnsafePointerView(denoPtr);
...
function toDenoFFIType(type) {
  switch (type) {
    ...
    case FFIType.napi_value:
      throw new Error(DENO_NAPI_UNSUPPORTED);
    default:
      return type;
  }
}
