...
const LoginStandardPage: Component = () => {
    ...
    return (<UKCard color={"filled"} class={styles.modal}>
        <UKText role={"title"} size={"l"} emphasized={true}>
            Login to {options.displayName}
        </UKText>
        ...
        {showTwoFactor() ? (<>
                ...
                onValueChange={async (val) => {
                    if (val.length === 6) {
                        const resp = await trpc.authorization.passwordSignin.mutate({
                            username: username(), password: password(), twoFactorCode: val,
                        });
            ...
            <form>
                ...
                <div class={styles.loginButtons}>
                    <UKButton
                        onClick={() => navigate(`/forgot-password?username=${username()}`)}
                        disabled={username() === ""}
                        color={"standard"}
                    >
                        Forgot password?
                        ...
                        onClick={async () => {
                            ...
                            if (resp.type === "success") {
                                ...
                                if (redirect) {
                                    ...
                                    return {
                                        state: AffirmativeButtonState.Success, cb: () => navigate(redirect)
                                    };
                                }
                                ...
                                return {
                                    state: AffirmativeButtonState.Success, cb: () => navigate("/app"),
                                };
                            }
