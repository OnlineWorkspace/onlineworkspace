...
export default class FilesystemSystem extends System {
  ...
  */
  registerFileExtension(extension: `.${string}`, displayName: string, type: FileMediaType) {
    ...
    this.log.debug(`Registered file extension ${this.log.emphasis(extension)} -> '${displayName}' as type ${this.log.emphasis(FileMediaType[type])}`);
  ...
  /**
   * Get a file's md5 hash as a hex string
  ...
  serveFile(userId: number, fsPath: string, isPublic: boolean = false, evadeCache = false, validMs: number = 21600000): string {
    if (!evadeCache) {
  ...
  override async startup(): Promise<boolean> {
    ...
    this.registerFileExtension(".png", "Portable Network Graphic (PNG)", FileMediaType.Image);
    this.registerFileExtension(".webp", "Web Picture format (WebP)", FileMediaType.Image);
    this.registerFileExtension(".avif", "AV1 Image File Format (AVIF)", FileMediaType.Image);
    this.registerFileExtension(".gif", "Graphics Interchange Format (GIF)", FileMediaType.Image);
    this.registerFileExtension(".jpeg", "Joint Photographic Experts Group (JPEG)", FileMediaType.Image);
    this.registerFileExtension(".jpg", "Joint Photographic Experts Group (JPEG)", FileMediaType.Image);
    this.registerFileExtension(".svg", "Scalable Vector Graphics (SVG)", FileMediaType.Image);
    this.registerFileExtension(".ico", "Icon format", FileMediaType.Image);

    this.registerFileExtension(".mp4", "MPEG-4 Video (MP4)", FileMediaType.Video);
    this.registerFileExtension(".webm", "WebM Video", FileMediaType.Video);
    this.registerFileExtension(".mkv", "Matroska Video (MKV)", FileMediaType.Video);
    this.registerFileExtension(".mov", "Apple QuickTime Movie", FileMediaType.Video);
    this.registerFileExtension(".avi", "Audio Video Interleave (AVI)", FileMediaType.Video);

    this.registerFileExtension(".obj", "Wavefront OBJ 3D Model", FileMediaType.ThreeDimensionalModel);
    this.registerFileExtension(".gltf", "GL Transmission Format (gLTF)", FileMediaType.ThreeDimensionalModel);
    this.registerFileExtension(".glb", "Binary GL Transmission Format (GLB)", FileMediaType.ThreeDimensionalModel);
    this.registerFileExtension(".fbx", "Filmbox 3D Asset (FBX)", FileMediaType.ThreeDimensionalModel);
    this.registerFileExtension(".stl", "Stereolithography 3D Asset (STL)", FileMediaType.ThreeDimensionalModel);

    this.registerFileExtension(".mp3", "MPEG Audio Layer III (MP3)", FileMediaType.Audio);
    this.registerFileExtension(".wav", "Waveform Audio File Format (WAV)", FileMediaType.Audio);
    ...
    this.registerFileExtension(".csv", "Comma-Separated Values (CSV)", FileMediaType.Text);
    this.registerFileExtension(".md", "Markdown Documentation", FileMediaType.Text);
    this.registerFileExtension(".json", "Javascript Object Notation", FileMediaType.Text);

    this.registerFileExtension(".pdf", "Portable Document Format (PDF)", FileMediaType.PDF);

    this.registerFileExtension(".zip", "ZIP Archive", FileMediaType.Archive);
    this.registerFileExtension(".tar", "Tarball Archive", FileMediaType.Archive);
    this.registerFileExtension(".gz", "Gzip Compressed Archive", FileMediaType.Archive);
    this.registerFileExtension(".rar", "Roshal Archive (RAR)", FileMediaType.Archive);
    this.registerFileExtension(".7z", "7-Zip Compressed Archive", FileMediaType.Archive);

    this.registerFileExtension(".docx", "Microsoft Word Document", FileMediaType.RichOfficeDocument);
    this.registerFileExtension(".doc", "Microsoft Word Document (Legacy)", FileMediaType.RichOfficeDocument);
    this.registerFileExtension(".xlsx", "Microsoft Excel Spreadsheet", FileMediaType.RichOfficeDocument);
    this.registerFileExtension(".xls", "Microsoft Excel Spreadsheet (Legacy)", FileMediaType.RichOfficeDocument);
    this.registerFileExtension(".pptx", "Microsoft PowerPoint Presentation", FileMediaType.RichOfficeDocument);
