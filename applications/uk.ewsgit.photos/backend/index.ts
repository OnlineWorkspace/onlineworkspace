...
instance.sys.event.on(WorkspacesEvent.QuarterHourly, async () => {
  for (const user of await instance.sys.users.getAllUsers()) {
    ...
    for (const imagePath of imageFiles) {
      ...
      if (exists.length === 0) {
        ...
        await db`INSERT INTO public.uk_ewsgit_photos_media (width, height, path, timestamp, owner_id) VALUES (${imageMetadata.width}, ${imageMetadata.height}, ${imagePath}, ${imageFileStats.mtime}, ${userId})`;
      }
    }
  }
  ...
  //   wasmBinaryPath: path.join(instance.sys.filesystem.getApplicationSrc("uk.ewsgit.photos")!, "backend/services/faceLandmarker/landmarker.wasm")
  // }, {
