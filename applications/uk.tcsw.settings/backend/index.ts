  ...
  overview: {
    user: {
      fullName: procedure.output(z.string()).query(async (opt) => {
        ...
        return (
          `${fullName?.forename} ${fullName?.surname || ""}` || "Unknown User"
        );
      }),
  ...
  customization: {
    wallpaper: {
      ...
      officialWallpapers: procedure
        ...
        .query(async (opt) => {
          ...
          for (const wallpaperName of await fs.readdir(
            officialWallpapersPath,
          )) {
      ...
      upload: procedure.input(octetInputParser).mutation(async (opt) => {
        ...
        // @ts-expect-error bytes does exist, but the types are wrong for some reason
      ...
      setOptions: procedure
        ...
        .mutation(async (opt) => {
          ...
          for (const resizedWallpaper of await fs.readdir(
            resizedWallpapersPath,
          )) {
    ...
    quickShortcuts: {
      ...
      resetToDefaults: procedure.mutation(async (opt) => {
        ...
        await a.setValue(opt.ctx.userId, a.defaultValue);
