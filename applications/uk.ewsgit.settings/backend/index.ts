    ...
    branding: {
      loginBanner: {
        preview: procedure
          ...
          .query(async (opt) => {
            const loginBannerCachePath = path.join(instance.sys.filesystem.FS_ROOT, `assets/login/banner.png`);
      ...
      loginBackground: {
        preview: procedure
          ...
          .query(async (opt) => {
            const loginBackgroundCachePath = path.join(instance.sys.filesystem.FS_ROOT, `assets/login/background.png`);
      ...
      favicon: {
        preview: procedure
          ...
          .query(async (opt) => {
            const loginBannerCachePath = path.join(instance.sys.filesystem.FS_ROOT, `assets/favicon.png`);
      ...
      squareLogo: {
        preview: procedure
          ...
          .query(async (opt) => {
            const loginBannerCachePath = path.join(instance.sys.filesystem.FS_ROOT, `assets/square_logo.png`);
  ...
  customization: {
    wallpaper: {
      ...
      getCurrentWallpaper: procedure.query(async (opt) => {
        ...
        return await opt.ctx.instance.sys.image.serveImage(opt.ctx.userId, requiredResizedWallpaperPath, {
          isPublic: false,
          evadeCache: true,
        });
      }),
    ...
    colorTheme: {
      getWallpaperPixelData: procedure.output(z.number().array()).query(async (opt) => {
        ...
        const buf = (
          await sharp(wallpaperPath).raw().toBuffer({
