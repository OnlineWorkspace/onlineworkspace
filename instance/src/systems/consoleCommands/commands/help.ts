...
export default class HelpCommand extends Command {
  override commandId = "help";
  flags = {};
  aliases = [];
  override shortDescription = "Display this help information";
  ...
  async run(parameters: ICommandRuntimeParameters) {
    ...
    this.instance.log.system.info(
      "-".repeat((LOG_WIDTH - HELP_BANNER_TITLE.length) / 2) + HELP_BANNER_TITLE + "-".repeat((LOG_WIDTH - HELP_BANNER_TITLE.length) / 2),
    );

    for (const command of this.instance.sys.consoleCommands.commands) {
      this.instance.log.system.info(
